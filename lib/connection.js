...
Connection.prototype.setupIOstreams = function () {
  var self = this;
  self.logger.warn('Connecting to %s', self.name);
  ...
  self.socket.on('connect', function () {
    ...
    self._nextResponse();
    self.emit('connect');
    self.logger.warn('Connected to %s', self.name);
  });
};
...
Connection.prototype._nextResponse = function () {
  ...
  self.in.readFields([
    ...
    debug('receiveResponse: got call#%s response from %s, flag: %s, size: %s', id, self.name, data.flag, data.size);
    ...
    self.in.readBytes(size, function (err, buf) {
      ...
      if (isError) {
        ...
        if (err.name !== 'org.apache.hadoop.hbase.regionserver.NoSuchColumnFamilyException') {
          self._close(err);
        }
      } else {
