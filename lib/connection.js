...
function Connection(remoteId) {
  ...
  this.pingInterval = remoteId.rpcTimeout || 60000; // 1 min; how often sends ping to the server in msecs
 ...
 */
Connection.prototype.receiveResponse = function () {
  // if (shouldCloseConnection.get()) {
  //   return;
  // }
  var self = this;
  ...
  self.in.readFields([
    ...
    self.in.readBytes(size, function (err, buf) {
      ...
      if (isError) {
        ...
        debug('call#%s error: %s', call.id, err.stack);
        call.setException(err);
        return;
      }

      // var value = ReflectionUtils.newInstance(valueClass, conf);
      var value = HbaseObjectWritable.readFields(io);
      debug('call#%s got (%s) %s', call.id, value.declaredClass, value.instance);
...
Connection.prototype.call = function (method, parameters, callback) {
 ...
 */
Connection.prototype.getClosestRowBefore = function (regionName, row, family, callback) {
  debug('getClosestRowBefore(`%s`, `%s`, `%s`)', regionName, row, family);
  this.call('getClosestRowBefore', [regionName, row, family], callback);
};

Connection.prototype.getProtocolVersion = function (protocol, clientVersion, callback) {
  ...
  this.call('getProtocolVersion', [protocol, clientVersion], callback);
};
 ...
 */
Call.prototype.setException = function (error) {
  this.error = error;
  this.callComplete();
};
