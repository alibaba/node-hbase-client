...
Connection.prototype._resolveAddress = function (callback) {
  if (IP_RE.test(this.address.host)) {
    return callback();
  }
  // resolve hostname first
  dns.lookup(this.address.host, function (err, ip) {
    if (err) {
      this.logger.warn('%s dns.lookup error: %s', this.name, err);
    }

    debug('%s dns.lookup %s => %s', this.name, this.address.host, ip);
    if (ip) {
      this.address._sourceHost = this.address.host;
      this.address.host = ip;
    }
    callback();
  }.bind(this));
};
...
Connection.prototype._cleanupCalls = function (err) {
  ...
  this.logger.warn('%s: clenaup %d calls, send "%s:%s" response.', this.name, count, err.name, err.message);
  // clean timer
  if (this._pingTimer) {
