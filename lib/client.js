    ...
    self.getHRegionConnection(metaLocation.getHostname(), metaLocation.getPort(), function (err, server) {
      ...
      function (err, regionInfoRow) {
        if (err) {
          ...
          if (err.name.toLowerCase().indexOf('offline') || err.message.toLowerCase().indexOf('offline') || 
              err.name.toLowerCase().indexOf('noserver') || err.message.toLowerCase().indexOf('noserver') ||
              err.name.toLowerCase().indexOf('notserving') || err.message.toLowerCase().indexOf('notserving')) {
            ...
            console.warn('[WARNNING] %d retries to locateRegion: %s', tries, metaKey.toString());
        ...
        if (Bytes.equals(parentTable, HConstants.META_TABLE_NAME)) {
          ...
          self.prefetchRegionCache(tableName, HConstants.EMPTY_START_ROW, function (err, count) {
            console.log('[%s, startRow:%s] prefetchRegionCache %d locations', tableName.toString(), '', count);
            if (err) {
              console.error('[prefetchRegionCache] error: %s', err.stack);
            }
          });
        }
