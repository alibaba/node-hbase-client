...
Client.prototype.processBatch = function (tableName, workingList, callback) {
  ...
  function buildDataStructs(row, i) {
    self.locateRegion(tableName, row.getRow(), true, function (err, loc) {
      ...
      var regionName = loc.getRegionInfo().regionNameStr;
      var actions = actionsByServer[regionName];
      if (!actions) {
        actions = actionsByServer[regionName] = [loc, new MultiAction()];
      }
      var action = new Action(row, i);
      // lastServers[i] = loc;
      actions[1].add(regionName, action);
