...
Client.prototype.getConnectionQueue = function (hostname, port, callback) {
  var self = this;
  var key = hostname + ':' + port;
  var queue = CONNECTION_QUEUE[key];
  if (!queue) {
    queue = CONNECTION_QUEUE[key] = new Bagpipe(1, {timeout: 1000});
    queue.on('full ', function (length) {
      self.logger.warn('connect cannot deal on time, queue jam, current queue length is:' + length);
    });
  }
  return queue;
};
