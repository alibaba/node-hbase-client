  ...
  self.locateRegion(tableName, row, useCache, function (err, location) {
    ...
    self.getRegionConnection(location.getHostname(), location.getPort(), function (err, server) {
      ...
      server[method](location.getRegionInfo().getRegionName(), obj, function (err, value) {
        ...
        if (err && err.name.indexOf('org.apache.hadoop.hbase.') >= 0) {
          retry++;
          self.logger.warn(err.stack);
          if (retry <= 3) {
            // max retry 3 times
            self.logger.warn('[%s] %s retries %s table row %s got wrong region: %s',
              method, retry, tableName, row.toString(), location.toString());
            return nextTick(self._action.bind(self, method, tableName, obj, false, retry, callback));
          }
        }
...
Client.prototype.ensureZookeeperTrackers = function (callback) {
  ...
  self.zk.once('connected', function (err) {
    if (err) {
      self.zkStart = 'error';
      self.logger.warn('[hbase-client] zookeeper connect error: %s', err.stack);
    ...
    self.zk.watch(self.rootRegionZKPath, function (err, value, zstat) {
      ...
      if (err) {
        self.logger.warn('[hbase-client] zookeeper watch error: %s', err.stack);
        if (firstStart) {
      ...
      self.logger.warn('[hbase-client] zookeeper start done, got new root %s, old %s',
        rootServer.servername, oldRootServer ? oldRootServer.servername : null);
      if (firstStart) {
