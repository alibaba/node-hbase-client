...
function Client(options) {
  ...
  if (typeof options.zookeeperRoot === "undefined" || options.zookeeperRoot === null) {
    options.zookeeperRoot = "/hbase";
  }
  if (typeof options.zookeeper === "object" && typeof options.zookeeper.quorum !== "undefined" && options.zookeeper.quorum !== null) {
...
Client.prototype.mput = function (tableName, rows, callback) {
  ...
  for (var i = 0, len = rows.length; i < len; i++) {
    if (!rows[i].row) {
      return callback('Object must have property row.');
    }
  }
  var workingList = [];
  for (var i = 0, len = rows.length; i < len; i++) {
    ...
    self.getRegionConnection(metaLocation.getHostname(), metaLocation.getPort(), function (err, server) {
      ...
      function (err, regionInfoRow) {
        ...
        if (Bytes.equals(parentTable, HConstants.META_TABLE_NAME)) {
          var eventName = Buffer.concat([tableName, regionInfo.startKey]);
          if (self._prefetchRegionCacheList[eventName]) {
              self.once(eventName, function(){callback(null, location);});
              return;
          }
