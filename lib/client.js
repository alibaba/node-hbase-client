...
Client.prototype.get = function (tableName, get, callback) {
  this._action('get', tableName, get, true, callback);
};

Client.prototype.put = function (tableName, put, callback) {
  this._action('put', tableName, put, true, callback);
};

Client.prototype.delete = function (tableName, del, callback) {
  this._action('delete', tableName, del, true, callback);
};


Client.prototype.getScanner = function (tableName, scan, callback) {
  this._action('openScanner', tableName, scan, true, function (err, scannerId, server) {
    if (err) {
      return callback(err);
    }
...
var nextTick = typeof setImmediate !== 'undefied' ? setImmediate : process.nextTick;

Client.prototype._action = function (method, tableName, obj, useCache, callback) {
  debug('action %s, useCache: %s', method, useCache);
  ...
  self.locateRegion(tableName, row, useCache, function (err, location) {
    ...
    self.getRegionConnection(location.getHostname(), location.getPort(), function (err, server) {
      ...
      server[method](location.getRegionInfo().getRegionName(), obj, function (err, value) {
        ...
        if (err && err.name.indexOf('WrongRegion') >= 0) {
          self.logger.warn('[%s] %s table row %s got wrong region: %s',
            method, tableName, row.toString(), location.toString());
          self.logger.warn(err.stack);
          return nextTick(self._action.bind(self, method, tableName, obj, false, callback));
        }
...
Client.prototype.processBatch = function (tableName, workingList, callback) {
  ...
  ep.after('multi_action', workingList.length, function () {
    ...
    function makeRequest(location, multiAction) {
      ...
      self.getRegionConnection(hostname, port, ep.done(function (server) {
        ...
        // var queue = self.getConnectionQueue(hostname, port);
        // queue.push(server.multi.bind(server), multiAction, ep.done(function (value) {
        //   processResult(value);
        //   ep.emit('request_done');
        // }));
      }));
  ...
  function buildDataStructs(row, i) {
    self.locateRegion(tableName, row.getRow(), true, ep.done(function (loc) {
      if (!loc) {
        // TODO: retry
...
Client.prototype.mget = function (tableName, rows, columns, callback) {
  ...
  this.processBatch(tableName, workingList, function (err, results) {
    if (err) {
      return callback(err);
    }
    ...
    self.getRegionConnection(metaLocation.getHostname(), metaLocation.getPort(), function (err, server) {
      ...
      function (err, regionInfoRow) {
        if (err) {
          ...
          if (errName.indexOf('offline') >= 0 || errName.indexOf('noserver') >= 0 || errName.indexOf('notserving') >= 0) {
            self.logger.warn(err.stack);
...
Client.prototype.getRegionConnection = function (hostname, port, callback) {
  ...
  server.once('close', function () {
    ...
    debug('%s closed, total %d connections', rsName, self.serversLength);
  });
};
