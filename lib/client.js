...
Client.prototype.getCachedLocation = function (tableName, row) {
  ...
  for (var i = 0; i < tableLocations.length; i++) {
    ...
    if (endKey.length === 0 || Bytes.compareTo(row, endKey) <= 0) {
      if (debug.enabled) {
        debug('locateRegionInMeta: get location(%s) from cache(%d: %d)', 
          location.toString(), tableLocations.length, i);
      }
  ...
  debug('locateRegionInMeta: miss from cache(%d)', tableLocations.length);
  return null;
