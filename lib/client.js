...
Client.prototype = {
  ...
  locateRegion: function (tableName, row, useCache, callback) {
    ...
    // }
    debug('locate table: %s', tableName.toString());
    var self = this;
  ...
  ensureZookeeperTrackers: function (callback) {
    var self = this;
    if (self.zkStart) {
      return callback();
    }
    self.zkStart = true;
    self.zk.start(function (err) {
      if (err) {
        return callback(err);
      }
  ...
  /**
    ...
    */
  locateRegionInMeta: function (parentTable, tableName, row, useCache, callback) {
    var location;
    ...
    // for (int tries = 0; true; tries++) {
      // if (tries >= numRetries) {
      //   throw new NoServerForRegionException("Unable to find region for " + Bytes.toStringBinary(row) + " after "
      //       + numRetries + " tries.");
      // }
    ...
    self.locateRegion(parentTable, metaKey, false, function (err, metaLocation) {
      ...
      if (!metaLocation) {
        // TODO: retries
        return callback(null, metaLocation);
      }
      ...
      self.getHRegionConnection(metaLocation.getHostname(), metaLocation.getPort(), function (err, server) {
        ...
        // if (Bytes.equals(parentTable, HConstants.META_TABLE_NAME) && (self.getRegionCachePrefetch(tableName))) {
   ...
   * @throws IOException
   */
  getHRegionConnection: function (hostname, port, callback, master) {
    if (master) {
      getMaster();
    }
    var server;
    ...
    // }
    var self = this;
    // self.ensureZookeeperTrackers();
    // See if we already have a connection (common case)
    server = this.servers[rsName];
    if (server) {
      debug('getHRegionConnection from servers cache, %s', rsName);
    ...
    server.on('connect', function () {
      ...
      server.getProtocolVersion(null, null, function (err, version) {
        ...
        debug('getHRegionConnection %s => getProtocolVersion: %s', rsName, version);
        // cache server
   ...
   */
  getCachedLocation: function (tableName, row) {
    ...
    for (var i = 0; i < tableLocations.length; i++) {
      ...
      if (Bytes.equals(endKey, HConstants.EMPTY_END_ROW) || Bytes.compareTo(row, location.regionInfo.endKey) <= 0) {
        return location;
      }
    }
  ...
  createServerName: function (data) {
...
Client.prototype.cacheLocation = function (tableName, location) {
  var startKey = location.getRegionInfo().getStartKey();
  var tableLocations = this.getTableLocations(tableName);
  var hostport = location.getHostnamePort();
  ...
  // }
  // if (hasNewCache) {
  //   LOG.debug("Cached location for " + location.getRegionInfo().getRegionNameAsString() + " is "
  //       + location.getHostnamePort());
  // }
};

Client.prototype.getRegionCachePrefetch = function (tableName) {
  return !this.regionCachePrefetchDisabledTables[Bytes.mapKey(tableName)];
};
 ...
 */
// Client.prototype.prefetchRegionCache = function (tableName, row) {
