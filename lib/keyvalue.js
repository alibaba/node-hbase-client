...
KeyValue.keyToString = function (b, o, l) {
  if (b === null) {
    return "";
  }
  var rowlength = Bytes.toShort(b, o);
  var row = Bytes.toStringBinary(b, o + Bytes.SIZEOF_SHORT, rowlength);
  var columnoffset = o + Bytes.SIZEOF_SHORT + 1 + rowlength;
  var familylength = b[columnoffset - 1];
  var columnlength = l - ((columnoffset - o) + KeyValue.TIMESTAMP_TYPE_SIZE);
  var family = familylength === 0 ? "" : Bytes.toStringBinary(b, columnoffset, familylength);
  var qualifier = columnlength === 0 ? "" : 
    Bytes.toStringBinary(b, columnoffset + familylength, columnlength - familylength);
  var timestamp = Bytes.toLong(b, o + (l - KeyValue.TIMESTAMP_TYPE_SIZE));
  var timestampStr = KeyValue.humanReadableTimestamp(timestamp);
  var type = b[o + l - 1];
...
KeyValue.prototype = {
  ...
  toString: function () {
    ...
    return KeyValue.keyToString(this.bytes, this.offset + KeyValue.ROW_OFFSET, this.getKeyLength()) + 
  ...
  getKey: function () {
    ...
    return this.bytes.slice(this.getKeyOffset(), keyLength);
  },
  ...
  getTimestamp: function () {
    if (this.timestampCache === -1) {
      var tsOffset = this.getTimestampOffset();
      this.timestampCache = Bytes.toLong(this.bytes, tsOffset);
    }
