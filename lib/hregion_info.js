...
function HRegionInfo(regionId, tableName, startKey, endKey, split) {
   ...
   */
  // public HRegionInfo(long regionId, final byte[] tableName, final byte[] startKey, final byte[] endKey, final boolean split,
  //     final long regionid) {
...
HRegionInfo.prototype = {
  ...
  setHashCode: function () {
    // var result = Arrays.hashCode(this.regionName);
   ...
   */
  containsRange: function (rangeStartKey, rangeEndKey) {
   ...
   */
  isSplitParent: function () {
    if (!this.isSplit()) {
      return false;
    }
    if (!this.isOffline()) {
      console.warn("Region is split but NOT offline: " + this.getRegionNameAsString());
    }
    return true;
  },
   ...
   */
  toString: function () {
    return "{" + HConstants.NAME + " => '" + this.regionNameStr + 
